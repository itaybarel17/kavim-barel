...
export const NavBar: React.FC = () => {
  ...
  const getNavItemClasses = (path: string, color: string) => {
    ...
    return `px-3 py-2 text-sm font-bold rounded border transition-all duration-200 ${isActive ? `bg-gradient-to-r from-${color}-500 to-${color}-600 text-white border-${color}-400 shadow-lg scale-105 ring-2 ring-${color}-300/50` : `bg-gradient-to-r from-${color}-500/80 to-${color}-600/80 text-white/90 border-${color}-400/80 hover:from-${color}-500 hover:to-${color}-600 hover:border-${color}-400 hover:text-white hover:shadow-sm`}`;
  };
  return <>
      {/* Mobile Header */}
      <nav className="lg:hidden flex justify-between items-center py-3 bg-gradient-to-r from-blue-600 to-blue-700 border-b shadow-lg px-0">
        <div className="flex items-center gap-3 px-0">
          <Button variant="ghost" size="sm" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="text-white hover:bg-white/10 p-2">
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && <div className="lg:hidden fixed inset-0 z-50 bg-black/50" onClick={() => setMobileMenuOpen(false)}>
          <div className="bg-white w-64 h-full shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b bg-gradient-to-r from-blue-600 to-blue-700">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setMobileMenuOpen(false)} className="text-white hover:bg-white/10 p-1">
                  <X size={18} />
                </Button>
              </div>
            </div>
            ...
            label,
            color
          }) => <Link key={path} to={path} onClick={() => setMobileMenuOpen(false)} className={`block w-full text-right ${getNavItemClasses(path, color)}`}>
                  {label}
                </Link>)}
      ...
      <nav className="hidden lg:flex justify-between items-center py-4 px-6 mb-8 bg-gradient-to-r from-blue-600 to-blue-700 border-b shadow-lg">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button variant="outline" size="sm" onClick={logout} className="bg-white/10 border-white/20 text-white hover:bg-white/20 hover:text-white transition-all duration-200">
              התנתקות
            </Button>
          </div>
        </div>
